...
X7S_LEFT_GRIPPER_PROFILE = GripperProfile(
    profile_id="x7s_left",
    label="X7S left hand",
    base_link_name="left_hand_link",
    link_names=("left_hand_link", "link12", "link13"),
    dof_defaults={
        "left_gripper1": (0.044, 0.0),
        "left_gripper2": (0.044, 0.0),
    },
    max_effort=100.0,
    output_usd_name="x7s_left_hand_gripper.usda",
)

G1_WBC_LEFT_GRIPPER_PROFILE = GripperProfile(
    profile_id="g1_wbc_left",
    label="G1 WBC left hand",
    base_link_name="left_wrist_yaw_link",
    link_names=(
        "left_wrist_yaw_link",
        "left_hand_palm_link",
    ...
    dof_defaults={
        "left_hand_thumb_0_joint": (0.0, 0.0),
        "left_hand_thumb_1_joint": (0.0, math.pi / 6.0),
        "left_hand_thumb_2_joint": (0.0, math.pi / 2.0),
        "left_hand_middle_0_joint": (0.0, -math.pi / 2.0),
        "left_hand_middle_1_joint": (0.0, -math.pi / 2.0),
        "left_hand_index_0_joint": (0.0, -math.pi / 2.0),
        "left_hand_index_1_joint": (0.0, -math.pi / 2.0),
    },
    max_effort=10.0,
    output_usd_name="g1_wbc_left_hand_gripper.usda",
)

GRIPPER_PROFILES = {
    "x7s_joint_left": X7S_LEFT_GRIPPER_PROFILE,
    "g1_wbc_left": G1_WBC_LEFT_GRIPPER_PROFILE,
}
...
def get_gripper_profile(profile_id: str | None) -> GripperProfile:
    if profile_id in GRIPPER_PROFILES:
        return GRIPPER_PROFILES[profile_id]
    available = ", ".join(sorted(GRIPPER_PROFILES))
    raise RuntimeError(f"Unsupported Grasp Editor robot profile '{profile_id}'. Available: {available}")


def robot_profile_id(pipeline) -> str | None:
    resolved_robot = getattr(pipeline, "_resolved_robot", None)
    profile = getattr(resolved_robot, "profile", None)
    profile_id = getattr(profile, "profile_id", None)
    if profile_id:
        return profile_id
    return getattr(pipeline.cfg, "robot_profile", None)
